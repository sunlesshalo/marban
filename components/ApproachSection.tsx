...
const PillarCard: React.FC<PillarProps> = ({ icon, title, description }) => (
  <div className="bg-brand-light p-8 group transition-all duration-300 hover:bg-brand-accent">
    <div className="flex items-center justify-center h-16 w-16 rounded-full bg-brand-primary text-brand-accent mb-6 group-hover:bg-white group-hover:text-brand-accent">
      {icon}
    </div>
    <h3 className="text-xl font-bold text-white mb-3">{title}</h3>
    <p className="text-brand-secondary group-hover:text-white">{description}</p>
  </div>
);
